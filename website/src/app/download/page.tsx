...
const platformIcons: Record<string, React.ReactNode> = {
  windows: (
    <svg className="h-8 w-8" viewBox="0 0 24 24" fill="currentColor">
      <path d="M3 5.5L10.5 4.4v7.1H3V5.5zm0 13V11.5h7.5V20.6L3 18.5zM11.25 4.2L21 2.5v9.2h-9.75V4.2zm0 17.3V12.7H21V22l-9.75-0.5z" />
    </svg>
  ),
  apple: (
    <svg className="h-8 w-8" viewBox="0 0 24 24" fill="currentColor">
    ...
    <div className="mx-auto max-w-6xl px-4 py-16 sm:px-6 sm:py-24">
      <div className="mx-auto max-w-2xl text-center">
        ...
        <p className="mt-4 text-lg text-muted">
          {SITE.name}를 사용하는 OS를 선택하세요. 정식 빌드는 GitHub Releases에서
          배포됩니다.
        </p>
      </div>
      ...
      <div className="mx-auto mt-16 grid max-w-4xl gap-6">
        {DOWNLOADS.map((item) => (
          <div
            key={item.platform}
            className="card-glow flex flex-col items-start gap-6 rounded-2xl p-6 sm:flex-row sm:items-center sm:justify-between"
          >
            <div className="flex items-center gap-4">
              <div className="rounded-xl bg-surface-elevated p-3 text-accent-blue">
            ...
            <div className="flex w-full flex-col items-stretch gap-2 sm:w-auto sm:items-end">
              {item.available ? (
                <Button href={SITE.releasesUrl} external>
                  다운로드
                </Button>
              ) : (
                <>
                  <span className="rounded-lg border border-border bg-surface px-4 py-2.5 text-center text-sm text-muted">
                    {item.note}
                  </span>
                  <Button href={SITE.releasesUrl} variant="ghost" external>
                    Releases 확인 →
                  </Button>
                </>
              )}
      ...
      <div className="mx-auto mt-16 max-w-2xl rounded-2xl border border-border bg-surface p-8">
        ...
        <p className="mt-2 text-sm text-muted">
          정식 빌드 전에 개발 버전을 직접 빌드할 수 있습니다.
        </p>
        <pre className="mt-4 overflow-x-auto rounded-xl border border-border bg-background p-4 font-mono text-sm leading-relaxed text-muted">
{`git clone https://github.com/sly22/kcode.git
